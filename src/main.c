...
int main(int argc, char *argv[]) {
    ...
    printf("=== Test0: Reallocation test ===\n");

    void *poke = sf_malloc(64);
    void *mon = sf_malloc(64);
    void *go = sf_malloc(64);
    sf_free(mon);
    sf_snapshot(true);
    sf_varprint(mon);


    void *w = sf_realloc(poke, 48);
    printf("Below is middle block poke:\n");
    printf("poke is at %pand go is at %p\n",poke, go);
    printf("Below is an allocated block of size 64\n");\
    sf_varprint(w);
    printf("poke\n");
    sf_varprint(poke);
    printf("mon\n");
    ...
    printf("=== Test0.5: Reallocation test and check contents===\n");

    int *a = sf_malloc(sizeof(int));
  	*a = 91362409;

  	int *b = sf_realloc(a, 320);
  	check_prim_contents(b, 91362409, "%d", "value1");
